...
- Solution Finder (`src/pages/solution-finder.tsx`): keyword search over `companyRecords`/`projects` via `findSolution` (now returns `matchedTerms` for the "Why this answer" chips — honest keyword-search labeling), session-only recent searches, follow-up question chips from source titles, Browse Company Brain library (text + classification filters, keyword chips re-run search), and a "Flag as knowledge gap" action on no-answer results (bound to the last executed query, `canSubmit`-gated, creates a pending `company-record` Recommendation routed to Carmen — never auto-approved). `addRecommendation` in the store has a defensive `canSubmit` guard.
