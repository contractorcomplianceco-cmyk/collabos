...
export default function MarketPulse() {
  const { marketSignals, competitors, marketPulseLoading } = useAppState();
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4 text-sm text-emerald-800">
        <span className="font-semibold">Public-source monitoring.</span> Signals are stored in the shared workspace database. No live external monitoring connection is active yet. Add competitors and keywords in{" "}
        <Link href="/settings" className="font-semibold underline">Settings</Link> to start tracking.
      </div>
      ...
      {marketPulseLoading ? null : filtered.length === 0 ? (
        <EmptyState
          ...
          hint="After you add competitors in Settings, capture or import signals to see movement here."
          action={<Link href="/settings" className="text-xs font-semibold text-emerald-600 hover:underline">Configure competitors</Link>}
        />
      ) : (
